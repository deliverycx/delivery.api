...
export class SmsAeroAxios extends Axios {
	...
	async smstable(body:{phone:string,textsms:string}){
		const urls = encodeURI(`https://cxcrimea@yandex.ru:zx4dUbwFtA319jZ3P90q7L2dyjtzD70M@gate.smsaero.ru/v2/sms/send?number=${body.phone}&text=${body.textsms}&sign=SKhinkalich`)
	}

	async smsautorization(body:{phone:string,textsms:string}){
		const urls = encodeURI(`https://cxcrimea@yandex.ru:zx4dUbwFtA319jZ3P90q7L2dyjtzD70M@gate.smsaero.ru/v2/sms/send?number=${body.phone}&text=${body.textsms}&sign=SKhinkalich`)
